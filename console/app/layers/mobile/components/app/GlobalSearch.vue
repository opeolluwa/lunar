...
const { searchQuery, closeSearch } = useMobileSearch();
...
<template>
  <div class="min-h-0 flex-1 overflow-y-auto bg-white pb-8 dark:bg-gray-900">
    <div
      v-if="!hasQuery"
    ...
    <template v-else>
      <div v-for="section in sections" :key="section.type" class="mb-3">
        <div class="flex items-center gap-2 px-6 pb-1 pt-5">
          <span
        ...
        <div
          class="mx-4 overflow-hidden rounded-2xl border border-gray-100 bg-white dark:border-gray-700 dark:bg-gray-800/60"
        >
          <button
