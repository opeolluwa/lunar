...
use crate::{
    ...
    entities::{recycle_bin, reminder, sync_queue},
    error::LunarError,
    repositories::{
...
pub trait ReminderRepositoryExt {
    ...
    ) -> Result<reminder::Model, LunarError>;

    async fn find_by_id(
        &self,
        identifier: &Uuid,
        meta: &Option<RequestMeta>,
    ) -> Result<Option<reminder::Model>, LunarError>;

    async fn find_all(
        &self,
        meta: &Option<RequestMeta>,
    ) -> Result<Vec<reminder::Model>, LunarError>;
    ...
    async fn extract_unsynced(&self) -> Result<Vec<reminder::Model>, LunarError>;
...
impl ReminderRepositoryExt for ReminderRepository {
    ...
    async fn update(
        ...
        let model = reminder::Entity::find()
            ...
            .await
            .map_err(|err| LunarError::DbOperationError(err.to_string()))?
            .ok_or_else(|| LunarError::DbOperationError("reminder not found".to_string()))?;
...
impl TransferRecord for ReminderRepository {
    async fn transfer_record(
        ...
        if !self
            ...
            .await?
        {
            return Err(LunarError::ReminderNotFound(record_identifier.to_string()));
        }
...
impl ReminderRepository {
    ...
    pub async fn create_js(&self, payload: JsValue, meta: JsValue) -> Result<JsValue, JsValue> {
        let payload: CreateReminder = serde_wasm_bindgen::from_value(payload).map_err(js_err)?;
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let model = <Self as ReminderRepositoryExt>::create(self, &payload, &meta).await?;
        to_js(&model)
    }
    ...
    pub async fn find_by_id_js(&self, identifier: &str, meta: JsValue) -> Result<JsValue, JsValue> {
        ...
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let model = <Self as ReminderRepositoryExt>::find_by_id(self, &id, &meta).await?;
        to_js(&model)
    }
    ...
    pub async fn find_all_js(&self, meta: JsValue) -> Result<JsValue, JsValue> {
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let models = <Self as ReminderRepositoryExt>::find_all(self, &meta).await?;
        to_js(&models)
    }
    ...
    pub async fn update_js(
        ...
        let id = Uuid::parse_str(identifier).map_err(js_err)?;
        let payload: UpdateReminder = serde_wasm_bindgen::from_value(payload).map_err(js_err)?;
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let model = <Self as ReminderRepositoryExt>::update(self, &id, &payload, &meta).await?;
        to_js(&model)
    }
    ...
    pub async fn delete_js(&self, identifier: &str, meta: JsValue) -> Result<JsValue, JsValue> {
        let id = Uuid::parse_str(identifier).map_err(js_err)?;
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        <Self as ReminderRepositoryExt>::delete(self, &id, &meta).await?;
