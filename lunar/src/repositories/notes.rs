...
use crate::{
    ...
    entities::{notes, recycle_bin, sync_queue},
    error::LunarError,
    repositories::{
...
pub trait NotesRepositoryExt {
    ...
    ) -> Result<notes::Model, LunarError>;

    async fn find_by_id(
        &self,
        identifier: &Uuid,
        meta: &Option<RequestMeta>,
    ) -> Result<Option<notes::Model>, LunarError>;

    async fn find_all(&self, meta: &Option<RequestMeta>) -> Result<Vec<notes::Model>, LunarError>;
    ...
    async fn recently_added(
        &self,
        meta: &Option<RequestMeta>,
    ) -> Result<Vec<notes::Model>, LunarError>;
    ...
    async fn extract_unsynced(&self) -> Result<Vec<notes::Model>, LunarError>;
...
impl NotesRepositoryExt for NotesRepository {
    ...
    async fn delete(
        ...
        let model = notes::Entity::find()
            ...
            .await
            .map_err(|err| LunarError::DbOperationError(err.to_string()))?
            .ok_or_else(|| LunarError::DbOperationError("note not found".to_string()))?;
...
impl NotesRepository {
    ...
    pub async fn create_js(&self, payload: JsValue, meta: JsValue) -> Result<JsValue, JsValue> {
        let payload: CreateNote = serde_wasm_bindgen::from_value(payload).map_err(js_err)?;
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let model = <Self as NotesRepositoryExt>::create(self, &payload, &meta).await?;
        to_js(&model)
    }
    ...
    pub async fn find_by_id_js(&self, identifier: &str, meta: JsValue) -> Result<JsValue, JsValue> {
        ...
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let model = <Self as NotesRepositoryExt>::find_by_id(self, &id, &meta).await?;
        to_js(&model)
    }
    ...
    pub async fn find_all_js(&self, meta: JsValue) -> Result<JsValue, JsValue> {
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let models = <Self as NotesRepositoryExt>::find_all(self, &meta).await?;
        to_js(&models)
    }
    ...
    pub async fn delete_js(&self, identifier: &str, meta: JsValue) -> Result<JsValue, JsValue> {
        let id = Uuid::parse_str(identifier).map_err(js_err)?;
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        <Self as NotesRepositoryExt>::delete(self, &id, &meta).await?;
    ...
    pub async fn recently_added_js(&self, meta: JsValue) -> Result<JsValue, JsValue> {
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let models = <Self as NotesRepositoryExt>::recently_added(self, &meta).await?;
        to_js(&models)
    }
    ...
    pub async fn update_js(
        ...
        let id = Uuid::parse_str(identifier).map_err(js_err)?;
        let payload: UpdateNote = serde_wasm_bindgen::from_value(payload).map_err(js_err)?;
        let meta: Option<RequestMeta> = serde_wasm_bindgen::from_value(meta).map_err(js_err)?;
        let model = <Self as NotesRepositoryExt>::update(self, &id, &payload, &meta).await?;
        to_js(&model)
    }
