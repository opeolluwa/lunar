...
use substring_replace::SubstringReplace;
...
pub async fn generate_syncular_migrations(
    backend: DbBackend,
    output_dir: &Path,
) -> Result<(), DbErr> {
    for migration in Migrator::migrations() {
        ...
        // {output_dir}/{migration_name}/
        let migration_dir = output_dir.join(name);

        fs::create_dir_all(&migration_dir).map_err(|e| DbErr::Custom(e.to_string()))?;

        // {output_dir}/{migration_name}/up.sql
        let path = migration_dir.join("up.sql");
